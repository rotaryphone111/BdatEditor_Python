...
def BdatEditor(file=None, game=None):
    sg.theme('LightGrey1')
    if not file:
        file, game = FileOpenMenu()
    if file == '':
        return 
    ...
    table_dicts = BdatReader.read_bdat_file(file, game)
    ...
    while True:
        try:
            ...
            if event == 'Save Table':
                ...
                #throwaway = pd.Series([np.uint32], index="test")
                #d = d.append(throwaway)
                ...
                write_bdat(table_dicts, file, game)
